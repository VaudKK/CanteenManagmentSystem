...
{
    public partial class FrmAllUses : Form
    {
        ...
        private async void FrmAllUses_Load(object sender, EventArgs e)
        {
            await  GetData("SELECT * FROM tblDailyUse WHERE Date = '" + DateTime.Now.ToString("yyyy-MM-dd") +"'");
        ...
        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string CommandText = "SELECT * FROM tblDailyUse WHERE STRFTIME('%Y-%m-%d',DATE) = STRFTIME('%Y-%m-%d','" + selectedDate + "')";
